...
RELEASE_NOTES = TARGET_ROOT / "docs" / "repodossier-follow-up-release-notes.md"
REPODOSSIER_VERSION = TARGET_ROOT / "docs" / "repodossier-follow-up-version-decision.md"
PATCHHARBOR_BUILD_SMOKE = TARGET_ROOT / "docs" / "patchharbor-release-build-smoke.md"
...
RELEASE_NOTE_PATHS = [
    ...
    "tests/test_repodossier_follow_up_release_notes.py",
]
...
class RepoDossierFollowUpReleaseNotesTests(unittest.TestCase):
    def test_follow_up_release_notes_doc_exists_and_records_plan_contract(self) -> None:
        self.assertTrue(RELEASE_NOTES.is_file())
        text = RELEASE_NOTES.read_text(encoding="utf-8")
        required = [
            "PATCHHARBOR.17b3 – RepoDossier follow-up release notes",
            "RepoDossier Follow-up Release Notes",
            "Add RepoDossier follow-up release notes",
    ...
    def test_follow_up_release_notes_match_current_repodossier_version(self) -> None:
        ...
        text = RELEASE_NOTES.read_text(encoding="utf-8")
        self.assertIn(f"Source version | `{version}`", text)

    def test_follow_up_release_notes_reference_patchharbor_version(self) -> None:
        name, version = _pyproject_name_and_version(TARGET_ROOT / "pyproject.toml")
        self.assertEqual(name, "patchharbor")
        text = RELEASE_NOTES.read_text(encoding="utf-8")
        self.assertIn(f"Related PatchHarbor version | `{version}`", text)

    def test_source_plan_defines_17b3_follow_up_release_notes_and_commit(self) -> None:
        source = _discover_source_repo()
        before = _source_status(source)

        plan = (source / "planning" / "milestones_migration.md").read_text(encoding="utf-8")
        idx = plan.find("PATCHHARBOR.17b3")
        self.assertGreaterEqual(idx, 0)
        window = plan[idx : idx + 400]
        self.assertIn("RepoDossier Follow-up Release Notes", window)
        self.assertIn("Add RepoDossier follow-up release notes", window)
        self.assertIn("PATCHHARBOR.17c1", plan)

        after = _source_status(source)
        self.assertEqual(after, before)

    def test_follow_up_release_notes_include_required_sections_and_source_wrapper_context(self) -> None:
        text = RELEASE_NOTES.read_text(encoding="utf-8")
        required = [
            "Release identity",
            "Summary",
    ...
    def test_follow_up_release_notes_do_not_claim_build_publish_or_metadata_changes(self) -> None:
        text = RELEASE_NOTES.read_text(encoding="utf-8")
        expected = [
    ...
    def test_related_docs_link_to_follow_up_release_notes(self) -> None:
        for path in [REPODOSSIER_VERSION, PATCHHARBOR_BUILD_SMOKE, PATCHHARBOR_RELEASE_NOTES, PUBLIC_READINESS]:
            text = path.read_text(encoding="utf-8")
            with self.subTest(path=path):
                self.assertIn("PATCHHARBOR.17b3 applied", text)
                self.assertIn("docs/repodossier-follow-up-release-notes.md", text)
                self.assertIn("tests/test_repodossier_follow_up_release_notes.py", text)

    def test_follow_up_release_notes_are_read_only_for_source(self) -> None:
        source = _discover_source_repo()
        before = _source_status(source)
    ...
    def test_follow_up_release_notes_do_not_store_private_local_values_or_fences(self) -> None:
